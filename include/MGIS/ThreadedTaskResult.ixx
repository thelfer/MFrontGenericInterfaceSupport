...
namespace mgis {
  ...
  ThreadedTaskResult<T>::ThreadedTaskResult(Args&&... args) {
    this->result.template emplace<T>(std::forward<Args...>(args...));
  ...
  ThreadedTaskResult<T>::operator bool() const {
    const auto initialized =
        mgis::holds_alternative<mgis::monostate>(this->result);
    return initialized && (this->eptr == nullptr);
  }
