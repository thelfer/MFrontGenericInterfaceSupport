 ...
 *
 * Based on MPark.Variant work (https://github.com/mpark/variant)
...
#ifndef LIB_MGIS_VARIANT_HXX
#define LIB_MGIS_VARIANT_HXX
...
#define MGIS_BUILTIN_ADDRESSOF
#endif

#if __has_builtin(__builtin_unreachable)
#define MGIS_BUILTIN_UNREACHABLE
#endif

#if __has_builtin(__type_pack_element)
#define MGIS_TYPE_PACK_ELEMENT
#endif

#if defined(__cpp_constexpr) && __cpp_constexpr >= 201304
#define MGIS_CPP14_CONSTEXPR
#endif

#if __has_feature(cxx_exceptions) || defined(__cpp_exceptions) || \
    (defined(_MSC_VER) && defined(_CPPUNWIND))
#define MGIS_EXCEPTIONS
#endif

#if defined(__cpp_generic_lambdas) || defined(_MSC_VER)
#define MGIS_GENERIC_LAMBDAS
#endif

#if defined(__cpp_lib_integer_sequence)
#define MGIS_INTEGER_SEQUENCE
#endif

#if defined(__cpp_return_type_deduction) || defined(_MSC_VER)
#define MGIS_RETURN_TYPE_DEDUCTION
#endif

#if defined(__cpp_lib_transparent_operators) || defined(_MSC_VER)
#define MGIS_TRANSPARENT_OPERATORS
#endif

#if defined(__cpp_variable_templates) || defined(_MSC_VER)
#define MGIS_VARIABLE_TEMPLATES
#endif

#if !defined(__GLIBCXX__) || __has_include(<codecvt>)  // >= libstdc++-5
#define MGIS_TRIVIALITY_TYPE_TRAITS
#endif
...
// <functional>
#ifdef MGIS_TRANSPARENT_OPERATORS
...
#pragma warning(disable : 4100)
      ...
      inline constexpr auto invoke(F &&f, As &&... as) RETURN(
          lib::forward<F>(f)(lib::forward<As>(as)...))
#ifdef _MSC_VER
#pragma warning(pop)
#endif

          template <typename B, typename T, typename D>
          inline constexpr auto invoke(T B::*pmv,
                                       D &&d) RETURN(lib::forward<D>(d).*pmv)

              template <typename Pmv, typename Ptr>
              inline constexpr auto invoke(Pmv pmv, Ptr &&ptr) RETURN(
                  (*lib::forward<Ptr>(ptr)).*pmv)
                  ...
                  inline constexpr auto invoke(T B::*pmf, D &&d, As &&... as)
                      RETURN((lib::forward<D>(d).*pmf)(lib::forward<As>(as)...))
                          ...
                          inline constexpr auto invoke(Pmf pmf,
                                                       Ptr &&ptr,
                                                       As &&... as)
                              RETURN(((*lib::forward<Ptr>(ptr)).*
                                      pmf)(lib::forward<As>(as)...))
...
#ifdef MGIS_RETURN_TYPE_DEDUCTION
...
#ifdef MGIS_EXCEPTIONS
    throw bad_variant_access{};
#else
    std::terminate();
#ifdef MGIS_BUILTIN_UNREACHABLE
      ...
      struct recursive_union {
        ...
        struct get_alt_impl {
          ...
          inline constexpr AUTO_REFREF operator()(V &&v) const
              AUTO_REFREF_RETURN(
                  get_alt_impl<I - 1>{}(lib::forward<V>(v).tail_))
        };
      ...
      struct base {
        ...
        inline static constexpr int visit_visitor_return_type_check() {
          static_assert(lib::all<std::is_same<F, Fs>::value...>::value,
                        "`mgis::visit` requires the visitor to have a single "
                        "return type.");
          return 0;
        }
        ...
        inline static constexpr AUTO make_dispatch(lib::index_sequence<Is...>)
            ...
            make_fdiagonal_impl() AUTO_RETURN(make_dispatch<F, Vs...>(
                lib::index_sequence<lib::indexed_type<I, Vs>::value...>{}))

                template <typename F, typename... Vs, std::size_t... Is>
                inline static constexpr AUTO
            make_fdiagonal_impl(lib::index_sequence<Is...>) AUTO_RETURN(
                make_farray(make_fdiagonal_impl<Is, F, Vs...>()...))
       ...
       private:
        ...
        struct visit_exhaustive_visitor_check {
          static_assert(lib::is_invocable<Visitor, Values...>::value,
                        "`mgis::visit` requires the visitor to be exhaustive.");
...
#define MGIS_VARIANT_RECURSIVE_UNION(destructible_trait, destructor)       \
  ...
  union recursive_union<destructible_trait, Index, T, Ts...> {             \
   ...
   private:                                                                \
    ...
    MGIS_VARIANT_RECURSIVE_UNION(Trait::TriviallyAvailable,
                                 ~recursive_union() = default;);
    MGIS_VARIANT_RECURSIVE_UNION(Trait::Available, ~recursive_union(){});
    MGIS_VARIANT_RECURSIVE_UNION(Trait::Unavailable,
                                 ~recursive_union() = delete;);

#undef MGIS_VARIANT_RECURSIVE_UNION
...
#define MGIS_VARIANT_DESTRUCTOR(destructible_trait, definition, destroy) \
  template <typename... Ts>                                              \
  class destructor<traits<Ts...>, destructible_trait>                    \
   ...
   protected:                                                            \
    ...
    MGIS_VARIANT_DESTRUCTOR(Trait::TriviallyAvailable, ~destructor() = default;
                            , inline void destroy() noexcept {
                              this->index_ = static_cast<index_t>(-1);
                            });

    MGIS_VARIANT_DESTRUCTOR(Trait::Available,
                            ~destructor() { destroy(); },
                            inline void destroy() noexcept {
    ...
    MGIS_VARIANT_DESTRUCTOR(Trait::Unavailable, ~destructor() = delete;
                            , inline void destroy() noexcept = delete;);

#undef MGIS_VARIANT_DESTRUCTOR
...
#define MGIS_VARIANT_MOVE_CONSTRUCTOR(move_constructible_trait, definition) \
  template <typename... Ts>                                                 \
...
#undef MGIS_VARIANT_MOVE_CONSTRUCTOR

    template <typename Traits, Trait = Traits::copy_constructible_trait>
    class copy_constructor;

#define MGIS_VARIANT_COPY_CONSTRUCTOR(copy_constructible_trait, definition) \
  template <typename... Ts>                                                 \
  class copy_constructor<traits<Ts...>, copy_constructible_trait>           \
   ...
   public:                                                                  \
    ...
    MGIS_VARIANT_COPY_CONSTRUCTOR(
        Trait::TriviallyAvailable,
        copy_constructor(const copy_constructor &that) = default;);

    MGIS_VARIANT_COPY_CONSTRUCTOR(Trait::Available,
...
#undef MGIS_VARIANT_COPY_CONSTRUCTOR
...
#define MGIS_VARIANT_MOVE_ASSIGNMENT(move_assignable_trait, definition) \
  template <typename... Ts>                                             \
...
#undef MGIS_VARIANT_MOVE_ASSIGNMENT

    template <typename Traits, Trait = Traits::copy_assignable_trait>
    class copy_assignment;

#define MGIS_VARIANT_COPY_ASSIGNMENT(copy_assignable_trait, definition)  \
  template <typename... Ts>                                              \
  class copy_assignment<traits<Ts...>, copy_assignable_trait>            \
   ...
   public:                                                               \
    ...
    MGIS_VARIANT_COPY_ASSIGNMENT(
        Trait::TriviallyAvailable,
        copy_assignment &operator=(const copy_assignment &that) = default;);

    MGIS_VARIANT_COPY_ASSIGNMENT(Trait::Available,
                                 copy_assignment &
...
#undef MGIS_VARIANT_COPY_ASSIGNMENT
  ...
  namespace detail {
    ...
    generic_get_if(V *v) noexcept AUTO_RETURN(
  ...
  inline constexpr bool operator>=(const variant<Ts...> &lhs,
    ...
    return rhs.valueless_by_exception() ||
           (!lhs.valueless_by_exception() &&
            (lhs.index() > rhs.index() ||
             (lhs.index() == rhs.index() &&
              variant::visit_value_at(lhs.index(), greater_equal{}, lhs,
                                      rhs))));
#endif
  }
...
namespace std {
  ...
  template <typename... Ts>
  struct hash<mgis::detail::enabled_type<
      mgis::variant<Ts...>,
      mgis::lib::enable_if_t<mgis::lib::all<mgis::detail::hash::is_enabled<
          mgis::lib::remove_const_t<Ts>>()...>::value>>> {
    using argument_type = mgis::variant<Ts...>;
    using result_type = std::size_t;

    inline result_type operator()(const argument_type &v) const {
      using mgis::detail::visitation::variant;
      std::size_t result =
...
#ifdef MGIS_GENERIC_LAMBDAS
                    [](const auto &alt) {
                      using alt_type = mgis::lib::decay_t<decltype(alt)>;
                      using value_type = mgis::lib::remove_const_t<
   ...
   private:
    ...
    struct hasher {
      ...
      inline std::size_t operator()(const Alt &alt) const {
        using alt_type = mgis::lib::decay_t<Alt>;
        using value_type =
            mgis::lib::remove_const_t<typename alt_type::value_type>;
        return hash<value_type>{}(alt.value);
      }
    };
#endif
  ...
  template <>
  struct hash<mgis::monostate> {
    using argument_type = mgis::monostate;
    using result_type = std::size_t;
