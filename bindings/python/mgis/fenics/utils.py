...
def vector_to_tensor(T):
    ...
    if ufl.shape(T) == (4,):
        ...
        return as_tensor([[T[0], T[3], T[5]], [T[4], T[1], T[7]], [T[6], T[8], T[2]]])
    else:
        raise NotImplementedError
...
def get_quadrature_element(cell, degree, dim=0):
    if dim in [0, 1, (), (0,), (1,), (0, 0), (0, 1), (1, 0), (1, 1)]:
        return FiniteElement("Quadrature", cell, degree=degree, quad_scheme="default")
    elif type(dim) == int:
        return VectorElement(
            "Quadrature", cell, degree=degree, dim=dim, quad_scheme="default"
        )
    elif len(dim) == 1 or (len(dim) == 2 and 0 in dim):
        d = [dd for dd in list(dim) if dd != 0][0]
        return VectorElement(
            "Quadrature", cell, degree=degree, dim=d, quad_scheme="default"
        )
    elif type(dim) == tuple:
        return TensorElement(
            "Quadrature", cell, degree=degree, shape=dim, quad_scheme="default"
        )
    else:
...
def interpolate_on_quadrature(f, degree):
    ...
    if Ve.family() == "Quadrature":
        ...
        if len(shape) == 1:
            shape = shape[0]
        Vge = Ve.__class__(
            "Quadrature", Ve.cell(), degree, shape, quad_scheme="default"
        )
...
def project_on_quadrature(f, mesh, degree):
    ...
    dx = Measure(
        "dx",
        domain=mesh,
        metadata={"quadrature_scheme": "default", "quadrature_degree": degree},
    )
    fg = Function(Vg)
...
def compute_on_quadrature(f, mesh, degree):
    ...
    fg = Function(Vg)
    if (
        isinstance(f, function.function.Function)
