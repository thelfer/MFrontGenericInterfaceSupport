/*!
 * \file   bindings/fenics/tests/ElasticityUniaxialTensileTestImposedStress-sxx.cxx
...
int main(){
  ...
  for (mgis::size_type i = 0; i != ux.size(); ++i) {
    // check strain values
    check(std::abs(ux[i] - e[i][0]) < eps, "invalid axial strain value");
    check(std::abs(e[i][1] + nu * e[i][0]) < eps,
          "invalid orthoradial strain value");
    check(std::abs(e[i][2] + nu * e[i][0]) < eps,
          "invalid orthoradial strain value");
    ...
    // check stress values
    check(std::abs(fx[i] - s[i][0]) < eps * yg, "invalid axial stress value");
    check(std::abs(s[i][0] - yg * e[i][0]) < eps * yg,
