...
//! \return a flag stating if registring an error is fatal, i.e. exits by calling `std::exit`.
...
InvalidResult ErrorBacktrace::registerErrorMessage(const ErrorReport e, const std::source_location &l) noexcept
{
  try {
    this->errorMessages_.push_back({l.file_name(), l.function_name(), l.line(), e});
  }
  catch (...) {
  }
...
#else
InvalidResult ErrorBacktrace::registerErrorMessage(const ErrorReport e) noexcept
{
  return this->registerErrorMessageWithoutSourceLocation(e);
}    // end of registerErrorMessage
#endif

InvalidResult ErrorBacktrace::registerErrorMessageWithoutSourceLocation(const ErrorReport e) noexcept
{
  try {
...
#ifdef MGIS_USE_SOURCE_LOCATION_INFORMATION
InvalidResult registerExceptionInErrorBacktrace(ErrorBacktrace &e, const std::source_location &l) noexcept
{
  try {
    throw;
  }
...
InvalidResult registerExceptionInErrorBacktraceWithoutSourceLocation(ErrorBacktrace &e) noexcept
{
  try {
    throw;
  }
  catch (std::exception &exception) {
    e.registerErrorMessageWithoutSourceLocation(std::string{exception.what()});
  }
  catch (...) {
