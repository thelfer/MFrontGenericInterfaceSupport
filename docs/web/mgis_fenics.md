...
* `state_variables`: a dictionary of internal and external state variables represented as `dolfin` objects
...
Either for the default or a user-defined one, the tangent operator associated with the residual is computed automatically using either the UFL symbolic derivation `ufl.derivative` for simple terms or using tangent operator blocks defined in the MFront behaviour for the nonlinear fluxes. More precisely, the variation of each flux is given by:

$$
\dfrac{\partial \bsig_i}{\partial u} = \sum_{j\in \text{blocks}(i)} \TT^{\bsig_i}_{\bg_j}\cdot \delta \bg_j(u)
$$
where the flux $\bsig_i$ is assumed to depend on the gradients $\bg_j$ for $j\in \text{blocks}(i)$ (which at least contains $i$ itself in general) and $\TT^{\bsig_i}_{\bg_j} = \dfrac{\partial \bsig_i}{\partial \bg_j}$ is the tangent operator associated with the corresponding block.
